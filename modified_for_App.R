...
#d<-rbind.fill(d, Fings2)
#new<-rbind.fill(WaterD, Fings3xFW)

# Sediment module -----------------------------------------------------
sedimentD = read.csv(file.choose()) #loads trainset 
...
PGR<-data.frame(LogDT50_Sediment = signif(predict(m4, newSed), 7)) #why the seven? perhaps read the QMRF and QPRF 

# Soil module -------------------------------------------------------------
soilD = read.csv(file.choose()) #load trainset 
...
KGB<-data.frame(LogDT50_Soil = signif(predict(m3, newSoil), 7)) #why the seven? perhaps read the QMRF and QPRF 
...
CIA<-data.frame(LogDT50_Freshwater = signif(predict(m3d2, newFW), 7)) #Predicted log(DT50). 
